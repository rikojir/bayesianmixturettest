...
# Figure 4
#### CUMULATIVE SCIENCE ####
newpar <- par(mfrow=c(1,1),mar=c(1.5,1.5,1.5,1.5))
set.seed(42)
start.time <- Sys.time()
maximumSmplSize=500
effSizePostModeVecSmallEffect<-numeric(maximumSmplSize-10)
effSizeLowerCIVecSmallEffect<-numeric(maximumSmplSize-10)
effSizeUpperCIVecSmallEffect<-numeric(maximumSmplSize-10)
firstComponent=rnorm(10,mean=3.5,sd=1.56)
secondComponent=rnorm(10,mean=2.89,sd=1.84)
for(smplsize in 10:maximumSmplSize){
  # Simulate two datasets which are identical
  firstComponent=c(firstComponent,rnorm(1,mean=3.5,sd=1.56))
  secondComponent=c(secondComponent,rnorm(1,mean=2.89,sd=1.84))
  
  # conduct the Gibbs sampling and isolate the posteriorMode as well as Lower and Upper CIs for the effect size
  resDF=bayes.t.test(10000,plot="none",firstComp = firstComponent, secondComp = secondComponent,hyperpars="wide",ci=0.95,burnin=5000,sd="sd",q=0.1)
  effSizePostModeVecSmallEffect[smplsize-9]=resDF$PosteriorExpectation[3]
  effSizeLowerCIVecSmallEffect[smplsize-9]=resDF$LowerCI[3]
  effSizeUpperCIVecSmallEffect[smplsize-9]=resDF$UpperCI[3]
} 

effSz=(2.89-3.5)/(sqrt((1.56^2+1.84^2)/2))
plot(effSizePostModeVecSmallEffect,pch=16,col="blue",lwd=1,ylim=c(-1,0.5),main="",ylab=expression(delta),xlab="")
points(effSizeLowerCIVecSmallEffect,pch=45,col="blue",lwd=1)
points(effSizeUpperCIVecSmallEffect,pch=45,col="blue",lwd=1)
abline(a=effSz,b=0,lwd=1,col="black")
abline(a=-0.2,b=0,lwd=1,col="grey",lty="dotted")
abline(a=-0.5,b=0,lwd=1,col="grey",lty="dotted")
#lines(x=c(0,0),y=c(-0.2,-0.5),ty="l",lty="dotted",lwd=1)
for(z in 1:maximumSmplSize){
  lines(x=c(z,z),y=c(effSizeLowerCIVecSmallEffect[z],effSizeUpperCIVecSmallEffect[z]),ty="l",lty="dotted",lwd=1)
}
end.time <- Sys.time()
time.taken <- end.time - start.time
time.taken